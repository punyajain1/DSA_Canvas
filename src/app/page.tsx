...
export default function Home() {
  ...
  const filteredData = useMemo(() => {
    return data.filter((q) => {
      ...
      const activeDiff = (difficultyOverrides[q.Title] || q.Difficulty).toUpperCase();
  ...
  const easyCount = progressData.filter(q => (difficultyOverrides[q.Title] || q.Difficulty).toUpperCase() === "EASY").length;
  const mediumCount = progressData.filter(q => (difficultyOverrides[q.Title] || q.Difficulty).toUpperCase() === "MEDIUM").length;
  const hardCount = progressData.filter(q => (difficultyOverrides[q.Title] || q.Difficulty).toUpperCase() === "HARD").length;

  const easyCompleted = progressData.filter(q => (difficultyOverrides[q.Title] || q.Difficulty).toUpperCase() === "EASY" && doneList[q.Title]).length;
  const mediumCompleted = progressData.filter(q => (difficultyOverrides[q.Title] || q.Difficulty).toUpperCase() === "MEDIUM" && doneList[q.Title]).length;
  const hardCompleted = progressData.filter(q => (difficultyOverrides[q.Title] || q.Difficulty).toUpperCase() === "HARD" && doneList[q.Title]).length;
                    ...
                    {paginatedData.map((q, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2.5 px-3 border-r border-[#2F2F2F]/40">
                            <span className={`inline-flex px-1.5 py-0.5 rounded-[3px] text-xs font-semibold tracking-wide ${getDifficultyColor(difficultyOverrides[q.Title] || q.Difficulty)}`} title={difficultyOverrides[q.Title] ? "Custom Difficulty" : "Default Difficulty"}>
                              {activeDiff.charAt(0).toUpperCase() + activeDiff.slice(1).toLowerCase()}
              ...
              <div className="mt-6 sm:mt-8 mb-10 w-full space-y-2">
                <div className="flex flex-col sm:flex-row sm:items-center text-[13px] sm:text-[14px]">
                  ...
                  <div className="relative group/diff">
                    <button className={`inline-flex items-center w-fit px-2 py-0.5 rounded-[3px] font-medium text-[12px] tracking-wide cursor-pointer hover:opacity-80 transition-opacity ${getDifficultyColor(difficultyOverrides[selectedQuestion.Title] || selectedQuestion.Difficulty)}`}>
                      {(() => {
                        const d = difficultyOverrides[selectedQuestion.Title] || selectedQuestion.Difficulty;
                ...
                <div className="flex flex-col sm:flex-row sm:items-center text-[13px] sm:text-[14px]">
                  ...
                  <button
                    ...
                    className={`inline-flex items-center w-fit gap-1.5 px-1.5 py-0.5 rounded-[3px] transition-colors ${doneList[selectedQuestion.Title]
